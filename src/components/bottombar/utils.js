...
function getColorsInCSSStr(str, type) {
  const colors = str
    ...
    .filter(
      (color) => colornames.indexOf(color) !== -1 || colorRegex.test(color)
    );
  return colors;
}
function getSVGColors(input, options) {
  return getSVGEl(input).then((svgEl) => {
    const fills = getEls("[fill]", svgEl).map((el) => el.getAttribute("fill"));
    const strokes = getEls("[stroke]", svgEl).map((el) =>
      el.getAttribute("stroke")
    );
    const stops = getEls("[stop-color]", svgEl).map((el) =>
    ...
    getEls("style", svgEl).forEach((styleEl) => {
    ...
    return {
      fills: compact(
        fills.filter(isColorString).map(toLowerCase).filter(unique)
      ),
      strokes: compact(
        strokes.filter(isColorString).map(toLowerCase).filter(unique)
      ),
      stops: compact(
        stops.filter(isColorString).map(toLowerCase).filter(unique)
      ),
    };
  });
}
